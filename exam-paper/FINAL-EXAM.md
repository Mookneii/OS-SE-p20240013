...
exact tree; only `p20240013` changes.

```text
~/os-se-p20240013/
├── os-lab-p20240013/        ← your existing labs (already committed)
├── os-class-activities-p20240013/   ← your existing activities
...
> **Starter templates.** The answer skeletons are already extracted in your home at
> **`~/exam-templates/`** (`README.md`, `commands.md`, `live_mods.md` — structure + the
> questions only, no answers, no commands). Copy them into your `final-exam/` folder and
> fill them in:
> `mkdir -p ~/<your-OS-SE-repo>/final-exam && cp ~/exam-templates/*.md ~/<your-OS-SE-repo>/final-exam/`
...
1. **D1 — The engine.** Write `buy_reactor_core`: takes a buyer name and a
   ...
   `150`, and on success decrements stock and appends a sales-log line
   that includes **your real student id**.
2. **D2 — The race.** Write `swarm`: launches `50` concurrent purchases in
   the background and waits. Reset stock to `150`, run several times, and
...
stock than the correct final value (with `150` stock and `50`
concurrent buyers)?
...
1. **E1 — `backup_project`.** Create a small sample project (a few files in a couple
   ...
   archive on each run and keep only the newest `2` archives, pruning the
   rest. Run it enough times that pruning happens.
   - 📸 `e1_backup_retention.png` — exactly `2` archives left + the log.
